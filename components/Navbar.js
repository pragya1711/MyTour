...
export default function Navbar() {
  ...
  const destinations = {
    ...
    "himachal pradesh": "/destinations/himachal",
    "uttar pradesh": "/destinations/up",
      ...
      <div className="max-w-7xl mx-auto flex justify-between items-center px-6 py-4">
        ...
        <div className="hidden md:block relative">
          ...
          {suggestions.length > 0 && (
            <div className="absolute w-full bg-white text-black mt-2 rounded-xl shadow-lg z-50">
              {suggestions.map((item, index) => (
                <Link
                  key={index}
                  to={destinations[item]}
                  className="block p-2 hover:bg-orange-100 cursor-pointer"
                >
                  {item}
                </Link>
              ))}
